...
class PolandSpecProvider(BaseSpecProvider):
    ...
    def nip(self) -> str:
        ...
        nip_digits += [self.random.randint(0, 9) for x in range(6)]
    ...
    def pesel(self,
        ...
        if not date_object:
            date_object = Datetime().datetime(1800, 2299)
        ...
        sum_v = sum(
            map(lambda x: x[0] * x[1], zip(pesel_coeffs, pesel_digits)))
    ...
    def regon(self) -> str:
        ...
        sum_v = sum(
            map(lambda x: x[0] * x[1], zip(regon_coeffs, regon_digits)))
        checksum_digit = sum_v % 11
        if checksum_digit > 9:
            checksum_digit = 0
