...
class RussiaSpecProvider(BaseSpecProvider):
    ...
    def snils(self) -> str:
        ...
        while int(reduce(lambda x, y: x + y,
                         map(lambda x: str(x), snils))) <= 1001998:
            snils = [self.random.randint(0, 9) for _ in range(9)]
        ...
        def get_snils_sum(snils):
            ...
            return ret_s(s)
        snils = reduce(lambda x, y: x + y, map(lambda x: str(x),
                                               snils)) + get_snils_sum(snils)
        return '{}-{}-{}-{}'.format(snils[:3],
                                    snils[3:6],
                                    snils[6:9],
                                    snils[9:])
